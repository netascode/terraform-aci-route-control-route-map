...
resource "aci_rest_managed" "rtctrlCtxP" {
  ...
  dn         = "${aci_rest_managed.rtctrlProfile.id}/ctx-${each.value.name}"
...
resource "aci_rest_managed" "rtctrlScope" {
  ...
  dn         = "${aci_rest_managed.rtctrlCtxP[each.value.name].id}/scp"
...
resource "aci_rest_managed" "rtctrlRsScopeToAttrP" {
  for_each   = { for ctx in var.contexts : ctx.name => ctx if ctx.set_rule != "" }
  dn         = "${aci_rest_managed.rtctrlScope[each.value.name].id}/rsScopeToAttrP"
...
resource "aci_rest_managed" "rtctrlRsCtxPToSubjP" {
  ...
  dn         = "${aci_rest_managed.rtctrlCtxP[each.value.ctx].id}/rsctxPToSubjP-${each.value.match_rule}"
